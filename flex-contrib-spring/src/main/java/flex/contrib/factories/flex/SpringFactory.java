package flex.contrib.factories.flex;
 ...
 * instances to the flex messaging framework.  To configure data services
 ...
 *
 *  &lt;beans&gt;
 *    &lt;bean id="weatherBean" class="dev.weather.WeatherService" singleton="true"/&gt;
 *  &lt;/beans&gt;
 * </pre>
 *
 ...
 * </pre>
 *
 * The value of the factory attribute is set to the id of the factory tag.  The value of
 * the source attribute is set to the id of the component in spring.
 * <p>
 * Note that spring 2 supports two lifecycles of components - singleton and non-singleton.
 * When you use the singleton attribute in your spring's bean tag definition, one instance
 * of the component is created that is reused for each invocation.  When you do not use
 * singleton, a new instance will be created for each invocation.  Though data services
 * supports a scope attribute the spring factory does not use that attribute.
 * <p>
 * This version of the spring factory has been updated for LiveCycle Data Services 2.5.
 * If your components implements the FlexConfigurable interface, the initialize method
 * on your component is called before it is returned.  Additionally some debug messages
 * have been added for the Configuration category so you can track the configuration of
 * spring factory instances.
 *
 * @version 0.2
 * @author Jeff Vroom, jvroom@adobe.com
 */
public class SpringFactory implements FlexFactory
{
    ...
    public FactoryInstance createFactoryInstance(String id, ConfigMap properties)
    {
        ...
        if (Log.isDebug())
            Log.getLogger("Configuration").debug(
                    "SpringFactory creating factory instance for destination: " + id + " with spring id: " + instance.getSource() + " with properties: " + properties);
    ...
    {
        Object applicationInstance;  // set for singleton's in spring
        ...
        {
            // If this is a singleton in spring, we know it will return the same instance.
            // though we could just call spring again, we only want to initialize it once
            // if it is a FlexConfigurable.
